...
export function TableMobile<T>({
  ...
  useEffect(() => {
    const updateScreenSize = () => {
      ...
      if (width < 375) {
        setScreenSize('xs'); // iPhone SE, iPhone 8/9, S24
      } else if (width < 640) {
  ...
  const getVisiblePages = () => {
    ...
    if (screenSize === 'xs') {
      maxVisible = 3; // iPhone SE, iPhone 8/9, S24
    } else if (screenSize === 'sm') {
      maxVisible = 4; // Dispositivos pequeños
    }
  ...
  const getResponsiveClasses = {
    ...
    // Table content
    contentPadding: screenSize === 'xs' ? 'pb-0' : screenSize === 'sm' ? 'pb-0' : 'pb-0',
    ...
    headerTextSize: screenSize === 'xs' ? 'text-xs' : screenSize === 'sm' ? 'text-xs' : 'text-xs',
    headerMinWidth: screenSize === 'xs' ? 'min-w-[60px]' : screenSize === 'sm' ? 'min-w-[70px]' : 'min-w-[70px] sm:min-w-[80px]',
    
    // Pagination
    paginatorPadding: screenSize === 'xs' ? 'px-2 py-2' : screenSize === 'sm' ? 'px-3 py-3' : 'px-3 sm:px-4 lg:px-6 py-3',
    paginatorMaxWidth: screenSize === 'xs' ? 'max-w-[98vw]' : screenSize === 'sm' ? 'max-w-[95vw]' : 'max-w-[95vw] sm:max-w-[90vw] md:max-w-[85vw] lg:max-w-[80vw] xl:max-w-[75vw]',
